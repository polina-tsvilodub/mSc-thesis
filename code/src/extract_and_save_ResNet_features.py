...
def extract_and_save_resnet_features(
    ...
    print("Number of imgs for feature extraction: ", len(data_loader.dataset.ids))
    for i in tqdm(range(len(data_loader.dataset.ids))): 
        resnet.eval()
        # Obtain the batch.
...
if __name__ == "__main__":
    extract_and_save_resnet_features(
        "../../data/train",
        "train",
        "COCO_train_ResNet_features_reshaped.pt",
    )
