...
In order to increase the similarity of target and distractor images and thereby increase the pressure to produce more specific messages in reference to the target \parencite[cf.][]{graf2016animal}, an experiment with similar image pairs was conducted on MS COCO. It also allowed to address \textbf{H5}. As described in Section \ref{ds:coco}, the pairs were constructed by picking the ones depicting at least the same three basic-level categories, as provided by the dataset annotations. Therefore, arguably the similariy of the images was conceptual, which also implied a certain degree of perceptual similarity. \pt{double check is this might be borne out in my image similarity data}. 
